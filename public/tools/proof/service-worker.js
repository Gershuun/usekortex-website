const CACHE = 'kortex-proof-v3';
const ASSETS = ['./', './index.html', './boot.mjs', './config.mjs', './icon.svg', './runtime.mjs', './styles.css'];
self.addEventListener('install', event => event.waitUntil(caches.open(CACHE).then(cache => cache.addAll(ASSETS))));
self.addEventListener('activate', event => event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key !== CACHE && key.startsWith('kortex-proof')).map(key => caches.delete(key))))));
